...
class NotificationManager:
    ...
    def show_install_success(self, app_name: str, app_path: str):
        ...
        notification = Gio.Notification.new(_("✓ {app_name} installed").format(app_name=app_name))
        notification.set_body(_("Successfully installed to {app_path}").format(app_path=app_path))
    ...
    def show_install_failure(self, app_name: str, error_message: str):
        ...
        notification = Gio.Notification.new(_("✗ Failed to install {app_name}").format(app_name=app_name))
    ...
    def show_update_available(self, app_name: str, current_version: str, new_version: str):
        ...
        notification = Gio.Notification.new(_("Update available for {app_name}").format(app_name=app_name))
        notification.set_body(_("Version {new_version} is available (current: {current_version})").format(new_version=new_version, current_version=current_version))
